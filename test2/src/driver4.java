...
public class driver4 {
	...
	static String xlPath, xlPath_Res;
	static String[][] xTC, xTS;
	static int xTC_r, xTC_c, xTS_r, xTS_c;
	static String vTCID, vTC_Execute, vTC_Res;
	static String vTSID, vKW, vEID, vTD, vTS_Res;
	
	public static void main(String[] args) throws Exception{
		// Read the Test Cases, Test Steps excel sheet
		xlPath = "C:\\Users\\Kosireddi\\Documents\\SLT June 2017\\KDF - Proj 2 - 2.xls";
		xlPath_Res = "C:\\Users\\Kosireddi\\Documents\\SLT June 2017\\KDF - Proj 2 - 2 - Res.xls";
			...
			if (vTC_Execute.equals("Y")){
				...
				for (int j=1; j<xTS_r; j++){
					...
					if (vTCID.equals(vTSID)){
						...
						getEID(vEID);
						try {
							vTS_Res = executeKW(vKW, vEID, vTD);
						} catch (Exception myErr) {
							System.out.println("Error occurred : " + myErr);
							vTS_Res = "Fail";
							xTS[j][8] = "Error is : " + myErr;
							File scrFile = ((TakesScreenshot)driver).getScreenshotAs(OutputType.FILE);
							// Now you can do whatever you need to do with it, for example copy somewhere
							FileUtils.copyFile(scrFile, new File("c:\\tmp\\screenshot_" + vTCID + vTSID + ".png"));
						}

						// Only when a TS fails, we will set TC to FAIL. Else we will leave it .
		...
		writeXL(xlPath_Res, "TestSteps", xTS);
	}
	
	public static String getEID(String fEID){
		// 1st 3 characters are @, then check and return the value
		// Else return as-is 
		if (true){
			return fEID;
		} else {
	...
	public static String executeKW(String fKW, String fEID, String fTD) throws NumberFormatException, InterruptedException {
		// Purpose : Calls the corresponding KW function and passes the parameters required. 
		// I/P : 3 i/p data
		// O/P : ???
		String fTS_Res = "Pass";
		if (fKW.equals("getUrl")){
			getUrl(fTD);
		}
		...
		if (fKW.equals("maximizeBrowser")){
			maximizeBrowser();
		}
		if (fKW.equals("clickLink")){
			clickLink(fEID);
		}
		...
		if (fKW.equals("sleepTime")){
			sleepTime(Long.parseLong(fTD));
		}
		if (fKW.equals("verifyText")){
			fTS_Res = verifyText(fEID, fTD);
		}
		if (fKW.equals("verifyElement")){
			fTS_Res = verifyElement(fEID);
		}
		return fTS_Res;
	...
	// Reusable Keyword Functions 
	  public static void navigateBrowser(String fData){
		  driver.navigate().to(fData);
	  }
	  public static void getUrl(String fData){
		  driver.get(fData);
	  }
	  
	  public static void sleepTime(long fData) throws InterruptedException{
		  Thread.sleep(fData);
	  }

	  
	  public static void openBrowser(String fData){
	  ...
	  public static void timeOutBrowser(int fData){
		  driver.manage().timeouts().implicitlyWait(fData, TimeUnit.SECONDS);
	  }
	  
	  public static void clickLink(String fEID){
	  ...
	  public static void typeText(String fEID, String fData){
		  driver.findElement(By.xpath(fEID)).clear();
		  driver.findElement(By.xpath(fEID)).sendKeys(fData);
	  }
	  
	  public static String verifyText(String fEID, String fData){
		  if (fData.equals(driver.findElement(By.xpath(fEID)).getText())){
	  ...
	  public static String verifyElement(String fEID){
		  if (driver.findElement(By.xpath(fEID)).isDisplayed()){
